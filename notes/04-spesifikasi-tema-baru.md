...
| Minimalis | Satu aksi primer per layar; ornamen dihapus; garis rambut 1 dp `NanzMusifyLineSoft`; spasi berbasis kelipatan 4 dp | Menghapus informasi (judul/artis/durasi tetap ada) |
| Warna tetap sama | Crimson `#E94560` dipertahankan sebagai `primary`; palet netral gelap/terang yang ada tetap jadi dasar | Mengganti `NanzMusifyCrimsonDefault` atau token publik |
...
Tambahkan ke `NanzMusifyPalette` — **nama token publik yang ada tidak boleh diganti**
...
**petakan** ke token NanzMusify (primary → `crimson`, surface → `surface`, dst.) supaya
komponen yang memakai token NanzMusify ikut berubah. Di bawah API 31, `MATERIAL_YOU`
...
| `TrackRow`, `TrackActions`, `Artwork`, `NanzMusifyPlayButton` | radius token, target sentuh ≥ 48 dp |
...
Ditambahkan `ui/theme/Shape.kt` (`NanzMusifyRadius` xs/sm/md/lg/xl/pill + `NanzMusifyShapes`
yang diikat ke `MaterialTheme.shapes` + `NanzMusifyChromeShape`), token baru di
`NanzMusifyPalette` (`surfaceTranslucent`, `hairline`, `accentSoft`, `scrimSheet`),
`ThemeMode.BLACK` + `nanzmusifyBlackPalette()` (#000 murni), dan `BrutalFrame` kini memakai
...
(cek: `grep -rn "RoundedCornerShape(" app/src/main/java/com/nanzmusify/nanas/ui/screens`).

**Fase 2 — Tema dinamis — ✅ SELESAI (gelombang tema 1).**
`dynamicColor` di `NanzMusifySettings` + `dynamicDarkColorScheme`/`dynamicLightColorScheme`
(di-guard `Build.VERSION_CODES.S`, otomatis diabaikan di API < 31), pemetaan balik ke
token lewat `nanzmusifyPaletteFromScheme()`, sakelar Material You di Settings (hanya tampil
...
mini player = radius `md` + `hairline`, lembar bawah = `scrimSheet` + `NanzMusifyRadius.top()`).
...
ukur `dumpsys gfxinfo com.nanzmusify.nanas framestats` sebelum/sesudah.

**Fase 4 — Motion iOS-like — 🟡 SEBAGIAN.**
Sudah: `ui/theme/Motion.kt` (`LocalReduceMotion`, `nanzmusifySpring`, `nanzmusifyTween`,
`nanzmusifyFade`, `nanzmusifyChromeEnter/Exit`, konstanta durasi 90/180/260/350 ms dan damping
...
(infinite pulse dimatikan), `NanzMusifyPlayButton`, `Crossfade` lirik di `NowPlayingScreen`.
...
2. Jangan mengganti nama token warna publik atau `NanzMusifyCrimsonDefault`.
