...
![NANZMUSIFY](docs/branding/nanzmusify-banner.png)

### *Hear What Words Can't Say*

**NANZMUSIFY** adalah aplikasi musik Android *audio-only* untuk YouTube Music — pencarian penuh, playlist, unduhan offline, pemutaran latar (background), dan rekomendasi radio otomatis, dibalut tema gelap‑crimson.
...
NANZMUSIFY memutar **hanya audio** dari YouTube Music (Opus/WebM atau AAC/M4A) sehingga lebih hemat kuota dibanding memutar video. Semua akses konten melalui *extractor* open source ([MetrolistExtractor], fork terpelihara dari NewPipeExtractor) — **tanpa API resmi YouTube dan tanpa akun**.

> ⚠️ **Disclaimer:** Proyek ini tidak berafiliasi dengan YouTube/Google. Pengguna bertanggung jawab mematuhi ketentuan layanan di wilayah masing‑masing. NANZMUSIFY hanya untuk pemutaran pribadi.
...
│                └─ Components: MiniPlayerBar, LyricsSheet, GenreReel, NanzMusifyPlayButton …
│
...
│
└─ Data ────────── Room DB · DataStore · model (NanzMusifyTrack, SearchFilter, …)
```
...
```
app/src/main/
├─ java/com/nanzmusify/nanas/
│  ├─ MainActivity.kt            # root Compose + NavHost
│  ├─ NanzMusifyApp.kt               # Application + init NewPipe
│  ├─ core/                      # ServiceLocator, LocaleHelper
│  ├─ data/
│  │  ├─ db/                     # Room (Daos, Entities, Database)
│  │  ├─ model/                  # NanzMusifyTrack, SearchFilter, NanzMusifyArchive
│  │  ├─ taste/                  # MusicTextAnalyzer, TasteRepository (profil selera)
│  │  ├─ settings/               # SettingsRepository
│  │  └─ LibraryRepository.kt
│  ├─ download/                  # DownloadService, NanzMusifyDownloadManager
...
### Clone
```bash
git clone https://github.com/Nanas/NanzMusify.git
cd NanzMusify
```
...
**NanzMusify tetap anonim**: tidak ada akun, tidak ada cookie, `setTokens()` tidak
...
> NanzMusify meniru spesifikasi klien, urutan tangga stream, dan pendekatan validasi URL
...
> kompatibel dan patuh. Salinan NanzMusify versi MIT yang sudah terdistribusi sebelum
