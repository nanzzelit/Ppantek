 ...
 */
package com.nanzmusify.nanas.local
...
import com.nanzmusify.nanas.data.model.NanzMusifyTrack.Companion.LOCAL_ID_PREFIX
...
class LocalMusicRepository(private val context: Context) {
    ...
    private val prefs by lazy {
        context.getSharedPreferences("nanzmusify_local", Context.MODE_PRIVATE)
    }
    ...
    suspend fun scanMediaStore(): List<NanzMusifyTrack> = withContext(Dispatchers.IO) {
        if (!hasPermission()) return@withContext emptyList()
        runCatching { queryStore() }.getOrDefault(emptyList())
    }

    private fun queryStore(): List<NanzMusifyTrack> {
        ...
        val out = mutableListOf<NanzMusifyTrack>()
    ...
    suspend fun scanTrees(): List<NanzMusifyTrack> = withContext(Dispatchers.IO) {
        val out = mutableListOf<NanzMusifyTrack>()
        trees().forEach { tree ->
    ...
    private fun walkTree(treeUri: Uri, out: MutableList<NanzMusifyTrack>) {
     ...
     */
    suspend fun enrichSafChunk(tracks: List<NanzMusifyTrack>): List<NanzMusifyTrack> =
    ...
    suspend fun embeddedArt(track: NanzMusifyTrack): String? = withContext(Dispatchers.IO) {
