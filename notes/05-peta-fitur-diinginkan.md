...
| # | Fitur diminta | Status | Di NanzMusify hari ini | Padanan di Meld | Usaha |
|---|---|---|---|---|---|
...
| 6 | Download & cache offline | **ADA** | `download/NanzMusifyDownloadManager.kt`, `download/HlsFlatDownloader.kt` (manifest → 1 file tanpa FFmpeg), `ui/screens/DownloadsScreen.kt`; menolak URL `validated = false` | `playback/{DownloadUtil,ExoDownloadService}.kt` | – |
| 7 | Search songs, albums, artists, videos, playlists | **PARSIAL** | `ui/screens/SearchScreen.kt` + `data/model/NanzMusifyTrack.kt` → `SearchFilter { ALL, SONGS, VIDEOS, ALBUMS, PLAYLISTS, PODCASTS }` — **tidak ada filter ARTISTS** | `ui/screens/search/{OnlineSearchScreen,OnlineSearchResult,LocalSearchScreen,SearchScreen}.kt`, `innertube/pages/{SearchPage,SearchSummaryPage,SearchSuggestionPage}.kt` | S (filter artis) + M (pencarian lokal) |
...
- **Konflik:** implementasi Meld memakai **server signaling**
  ...
  `MessageCodec.kt`, `ListenTogetherClient.kt`). NanzMusify berdiri di atas prinsip
