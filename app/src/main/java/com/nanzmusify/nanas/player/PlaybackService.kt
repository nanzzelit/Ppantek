 ...
 *
 * Catatan versi: NanzMusify memakai media3 1.10.1, jadi
...
import com.nanzmusify.nanas.MainActivity
import com.nanzmusify.nanas.player.audio.LoudnessStore
import com.nanzmusify.nanas.player.audio.SilenceDetectorAudioProcessor
...
/** Aksi kustom yang dikirim widget "NanzMusify Offline" untuk memutar antrean
 *  offline langsung dari [PlaybackService], tanpa perlu app dibuka lebih dulu. */
const val ACTION_PLAY_OFFLINE_QUEUE = "com.nanzmusify.nanas.action.PLAY_OFFLINE_QUEUE"
const val EXTRA_CATEGORY_ID = "com.nanzmusify.nanas.extra.CATEGORY_ID"
const val EXTRA_START_INDEX = "com.nanzmusify.nanas.extra.START_INDEX"
...
class PlaybackService : MediaSessionService() {
    ...
    /**
     * Rantai processor audio: detektor hening NanzMusify → pembuang hening media3 →
     * Sonic (kecepatan & nada). Processor kustom NanzMusify harus berada SEBELUM dua processor bawaan itu.
     */
    ...
    private fun observeAudioSettings(locator: com.nanzmusify.nanas.core.ServiceLocator, player: ExoPlayer) {
        serviceScope.launch {
     ...
     */
    private fun observeNormalization(locator: com.nanzmusify.nanas.core.ServiceLocator, player: ExoPlayer) {
    ...
    /**
     * Jalur khusus widget "NanzMusify Offline": widget mengirim intent ini
     * (bukan lewat [MediaSession]/[PlayerManager]) supaya lagu bisa langsung
     * diputar walau app-nya belum pernah dibuka sama sekali di sesi ini —
     * hanya butuh service ini hidup, tidak butuh MediaController tersambung.
     */
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        val categoryId = intent?.takeIf { it.action == ACTION_PLAY_OFFLINE_QUEUE }
            ?.getStringExtra(EXTRA_CATEGORY_ID)
        if (categoryId != null) {
            val startIndex = intent.getIntExtra(EXTRA_START_INDEX, 0)
            val locator = (application as NanzMusifyApp).locator
            serviceScope.launch {
                val tracks = withContext(Dispatchers.IO) { locator.library.offlineTracksFor(categoryId) }
                if (tracks.isEmpty()) return@launch
                val idx = startIndex.coerceIn(0, tracks.lastIndex)
                val player = mediaSession?.player ?: return@launch
                runCatching {
                    player.setMediaItems(tracks.map { it.toPlayableMediaItem() }, idx, 0L)
                    player.prepare()
                    player.play()
                }
            }
