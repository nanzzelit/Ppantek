...
| Modul | Berkas | Isi | Relevansi NanzMusify |
|---|---|---|---|
...
ktor+okhttp, kotlinx.serialization. **NanzMusify berbeda**: media3 1.10.1, okhttp 5.1.0,
`org.json`, Room+KSP, Coil 3, Rhino. Jadi porting = terjemahkan, bukan salin-tempel.

---

## 1. ✅ Lisensi — diputuskan: NanzMusify kini GPL-3.0-only (2026-09-06)

| | Lisensi |
|---|---|
| NanzMusify (`LICENSE`, header SPDX per berkas) | **GPL-3.0-only** ← sebelumnya MIT |
...
**Keputusan pemilik proyek:** *"Rilesensi mit ke 3.0"* — NanzMusify direlisensi dari MIT ke
GPL-3.0-only supaya **boleh menyerap kode Meld apa adanya**. Konsekuensi yang diterima:
setiap turunan/rilis NanzMusify tunduk copyleft (kode sumber harus tersedia bagi penerima),
...
1. **Menyalin berkas Meld diperbolehkan**, dengan syarat: pertahankan header hak cipta
   ...
   package/impor ke `com.nanzmusify.nanas`.
2. **Setiap berkas baru NanzMusify** memakai header SPDX yang sama:
   ```kotlin
   /*
...
- `app/.../utils/sabr/EjsNTransformSolver.kt`, `utils/cipher/CipherDeobfuscator.kt` —
  ...
  (VISIONOS/ANDROID_VR/IOS mengirim URL polos). NanzMusify punya `SignatureDecipher.kt`
...
- `playback/SleepTimer.kt` + `app/src/test/.../SleepTimerFadeTest.kt` → fade-out
  (NanzMusify sudah punya timer, belum punya fade).
...
## 4. Apa yang sudah diporting ke NanzMusify (commit `df7531a`, CI hijau)
...
| Dari Meld | Ke NanzMusify | Bentuk porting |
|---|---|---|
...
Sejak NanzMusify berlisensi GPL-3.0-only (§1), kode Meld boleh diadaptasi langsung dengan
header hak cipta asal + komentar path sumber. Yang sudah dipakai:

| Dari Meld (GPL-3.0) | Ke NanzMusify | Catatan adaptasi |
|---|---|---|
...
| `lyrics/LyricsProvider.kt` | `lyrics/LyricsProvider.kt` | tanpa `Context`/Hilt; hasil berupa `LyricsResult` NanzMusify |
...
3. Mau fitur baru → cari berkasnya di §3, baca **KDoc-nya dulu** (sering berisi
   jebakan yang sudah mereka bayar), baru reimplementasi dengan gaya NanzMusify.
...
5. Sebelum menyalin kode: ikuti syarat §1 (header hak cipta asal + komentar path sumber +
   header SPDX NanzMusify), lalu catat di §4.
