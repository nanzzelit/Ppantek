...
ui/
├─ theme/
│  ├─ Color.kt   NanzMusifyPalette + token publik (NanzMusifyBackground, NanzMusifySurface,
│  │             NanzMusifyElevated, NanzMusifyTextPrimary/Secondary/Muted/Bright,
│  │             NanzMusifyLine, NanzMusifyLineSoft, NanzMusifyCrimson, NanzMusifyRose, NanzMusifyScrim)
│  ├─ Theme.kt   enum ThemeMode { SYSTEM, DARK, LIGHT } · NanzMusifyTheme(themeMode,
│  │             accentArgb, fontKey) · darkScheme()/lightScheme() Material 3
│  └─ Type.kt    nanzmusifyTypography(family) — satu fungsi, ikut setelan fontKey
├─ components/   Artwork · BlurryBackdrop · DonateDialog · EventCountdown ·
│                GenreReelSlider · IntroOverlay · NanzMusifyPlayButton · LyricsSheet ·
...
(`rememberNavController` + `NanzMusifyNavHost` di `MainActivity.kt`), `enableEdgeToEdge()`
...
1. **Jangan menulis `Color(0xFF…)` di layar/komponen.** Pakai token
   (`NanzMusifySurface`, `NanzMusifyTextSecondary`, …) atau `MaterialTheme.colorScheme`.
   Palet di-provide lewat `LocalNanzMusifyPalette` (`staticCompositionLocalOf`), jadi
...
2. **Warna identitas tidak berubah:** crimson `#E94560`
   (`NanzMusifyCrimsonDefault`) adalah merek. Aksen kustom pengguna masuk lewat
...
3. **Baca token di composable scope, jangan di lambda draw.** Contoh benar ada di
   `BlurryBackdrop.kt`: `val background = NanzMusifyBackground` dulu, baru dipakai di
   `drawBehind`. Melanggar ini = crash/`@ReadOnlyComposable` error.
4. Menambah token baru: tambahkan field di `NanzMusifyPalette`, isi di
   `nanzmusifyDarkPalette()` **dan** `nanzmusifyLightPalette()` (plus varian BLACK nanti),
...
- Kartu hari ini lahir dari **satu** primitif: `BrutalFrame` di `Primitives.kt`
  (`clip(RoundedCornerShape(16.dp))` + `background(NanzMusifySurface)` +
  `border(1.dp, NanzMusifyLine)`). Ini titik ungkit tema baru: ubah di sini, hampir
  seluruh kartu ikut.
...
- `NanzMusifyScrim` sudah ada di palet — pakai itu, jangan mengarang alpha sendiri.
...
- [x] Setelan `reduceMotion` **sudah dihormati** (sejak gelombang tema 1):
      `NanzMusifyTheme` menyediakan `LocalReduceMotion` — gabungan preferensi pengguna
...
- [ ] **Aturan wajib untuk animasi baru:** jangan panggil `tween(...)` / `spring(...)`
      mentah. Pakai helper di `ui/theme/Motion.kt`: `nanzmusifyTween()`, `nanzmusifySpring()`,
      `nanzmusifyFade()`, `nanzmusifyChromeEnter()/Exit()` — semuanya runtuh jadi ~instan saat
...
- [ ] Spec yang dipakai di dalam `LaunchedEffect` harus **diambil di luar** effect
      (helper tema bersifat `@Composable`) — lihat `NanzMusifyPlayButton.morphSpec`.

**Cara mengukur (jangan menebak)**
```bash
# jejak frame saat scroll/berpindah layar
adb shell dumpsys gfxinfo com.nanzmusify.nanas framestats
...
## 8. Jebakan yang sudah pernah terjadi di UI NanzMusify
