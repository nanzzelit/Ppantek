...
// ------------------------------------------------------------------
// Gerakan NANZMUSIFY — lihat notes/04-spesifikasi-tema-baru.md §Gerak
//
...
@Composable
fun <T> nanzmusifySpring(
    dampingRatio: Float = NanzMusifyMotion.dampingCalm,
    stiffness: Float = NanzMusifyMotion.stiffnessGentle,
): SpringSpec<T> =
...
@Composable
fun <T> nanzmusifyTween(
    durationMs: Int = NanzMusifyMotion.normal,
    delayMs: Int = 0,
): TweenSpec<T> =
    if (LocalReduceMotion.current) {
...
@Composable
fun nanzmusifyFade(durationMs: Int = NanzMusifyMotion.normal): TweenSpec<Float> = nanzmusifyTween(durationMs)

/** Masuknya chrome (mini player, bar bawah): geser + pudar, atau pudar saja. */
@Composable
fun nanzmusifyChromeEnter(durationMs: Int = NanzMusifyMotion.slow): EnterTransition =
    if (LocalReduceMotion.current) {
        fadeIn(tween(0))
    } else {
        slideInVertically(nanzmusifyTween<IntOffset>(durationMs)) { it } + fadeIn(nanzmusifyTween(durationMs))
    }

/** Keluarnya chrome — pasangan [nanzmusifyChromeEnter]. */
@Composable
fun nanzmusifyChromeExit(durationMs: Int = NanzMusifyMotion.normal): ExitTransition =
    if (LocalReduceMotion.current) {
        fadeOut(tween(0))
    } else {
        slideOutVertically(nanzmusifyTween<IntOffset>(durationMs)) { it } + fadeOut(nanzmusifyTween(durationMs))
    }
...
fun nanzmusifyScreenEnter(reduceMotion: Boolean): EnterTransition =
    if (reduceMotion) {
        fadeIn(tween(0))
    } else {
        slideInHorizontally(
            spring(dampingRatio = NanzMusifyMotion.dampingCalm, stiffness = NanzMusifyMotion.stiffnessBrisk),
        ) { it / 6 } + fadeIn(tween(NanzMusifyMotion.slow))
    }

/** Layar lama keluar (maju): geser sedikit ke kiri + pudar. */
fun nanzmusifyScreenExit(reduceMotion: Boolean): ExitTransition =
    if (reduceMotion) {
        fadeOut(tween(0))
    } else {
        slideOutHorizontally(
            spring(dampingRatio = NanzMusifyMotion.dampingCalm, stiffness = NanzMusifyMotion.stiffnessBrisk),
        ) { -it / 12 } + fadeOut(tween(NanzMusifyMotion.normal))
    }

/** Layar sebelumnya kembali terlihat (mundur): geser dari kiri + pudar. */
fun nanzmusifyScreenPopEnter(reduceMotion: Boolean): EnterTransition =
    if (reduceMotion) {
        fadeIn(tween(0))
    } else {
        slideInHorizontally(
            spring(dampingRatio = NanzMusifyMotion.dampingCalm, stiffness = NanzMusifyMotion.stiffnessBrisk),
        ) { -it / 12 } + fadeIn(tween(NanzMusifyMotion.slow))
    }

/** Layar aktif keluar (mundur): geser ke kanan + pudar. */
fun nanzmusifyScreenPopExit(reduceMotion: Boolean): ExitTransition =
    if (reduceMotion) {
        fadeOut(tween(0))
    } else {
        slideOutHorizontally(
            spring(dampingRatio = NanzMusifyMotion.dampingCalm, stiffness = NanzMusifyMotion.stiffnessBrisk),
        ) { it / 6 } + fadeOut(tween(NanzMusifyMotion.normal))
    }
