...
#   tangga klien NanzMusify perlahan basi tanpa ada yang menyadari.
#
...
OURS="app/src/main/java/com/nanzmusify/nanas/yt/innertube/PlayerClientLadder.kt"
...
def ours_constants(src):
    """{NAMA_KONSTANTA: nilai string} — UA di NanzMusify disimpan sebagai konstanta."""
    consts = {}
...
drift = []
print("== Rantai stream Meld vs tangga NanzMusify ==")
for name in CHAIN:
    ...
    if match:
        ...
        if diffs:
            ...
            print(f"  {name:22s} = {cn} {cv} (kunci NanzMusify: {match.get('key')})")
    else:
        ...
        print(f"  {name:22s} ! hulu {cn} {cv} — NanzMusify punya versi: {ours_ver}")
        drift.append((name, f"versi beda (hulu {cv} vs NanzMusify {ours_ver})"))
...
for name in ("WEB", "WEB_REMIX", "WEB_CREATOR", "MOBILE", "ANDROID_VR_NO_AUTH", "ANDROID_VR_1_61_48"):
    ...
    if match:
        ...
        same_name = [s for s in our_specs if s.get("clientName") == cn]
        ours_ver = ", ".join(sorted({s.get("clientVersion", "?") for s in same_name})) or "-"
        note = " (tidak dipakai NanzMusify)" if not same_name else ""
        print(f"  {name:22s} ~ hulu {cn} {cv} — NanzMusify: {ours_ver}{note}")
        if same_name:
            drift.append((name, f"versi klien diagnostik beda (hulu {cv} vs NanzMusify {ours_ver})"))
