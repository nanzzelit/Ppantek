 ...
 */
package com.nanzmusify.nanas.yt
        ...
        OkHttpClient.Builder()
            ...
            .addInterceptor { chain ->
                ...
                if (url.host.endsWith("googlevideo.com")) {
                    ...
                    // 403 padahal URL-nya sah — dan NanzMusify dulu menebak dari `c=` saja lalu
                    ...
                    val specUa = com.nanzmusify.nanas.yt.innertube.PlayerClientLadder
...
data class ResolvedAudio(
     ...
     * di URL). Dipakai [com.nanzmusify.nanas.yt.innertube.StreamUrlValidator] untuk
...
data class YtTrackBundle(
    val track: NanzMusifyTrack,
    val related: List<NanzMusifyTrack>,
...
class YouTubeRepository {
    ...
    companion object {
        ...
        /** Dipanggil sekali dari NanzMusifyApp.onCreate. */
        fun initNewPipe() {
            if (initialized) return
            synchronized(this) {
                if (initialized) return
                NewPipe.init(
                    OkHttpDownloader.init(NanzMusifyHttp.extractClient),
                    Localization("en", "US"),
                )
                // Knob extractor: batas tunggu player response dinaikkan (default
                // fork 5 s terlalu pendek di jaringan seluler) dan panggilan
                // dislike pihak ketiga dimatikan (tidak dipakai NanzMusify — satu
                ...
                // NanzMusify selalu ANONIM: `ServiceList.YouTube.setTokens()` sengaja
    ...
    suspend fun bundle(videoId: String): YtTrackBundle {
        ...
        val seedHashtags = com.nanzmusify.nanas.data.taste.MusicTextAnalyzer
            .hashtagsOf(descText + " " + seedTags.joinToString(" "))
        return YtTrackBundle(track, related, seedTags, seedHashtags, seedCategory)
    }

    suspend fun relatedOf(videoId: String): List<NanzMusifyTrack> {
    ...
    suspend fun searchTracks(query: String, limit: Int = 20): List<NanzMusifyTrack> =
    ...
    fun resolveAudioBlocking(
        ...
        if (!com.nanzmusify.nanas.yt.innertube.PlayerClientLadder.extractorBypassed()) {
            val info = try {
                StreamInfo.getInfo(ServiceList.YouTube, "https://www.youtube.com/watch?v=$videoId")
            } catch (e: Exception) {
            ...
            // tidak ada pilihan lain — NanzMusify memutarnya lewat media3-exoplayer-hls.
            ...
            if (streams.isEmpty()) {
                ...
                com.nanzmusify.nanas.yt.innertube.PlayerClientLadder.push(
            ...
            com.nanzmusify.nanas.yt.innertube.PlayerClientLadder.noteExtractorSuccess()
            ...
            val mime = if (manifest) {
                com.nanzmusify.nanas.yt.innertube.InnertubeFallback.HLS_MIME
            } else {
            ...
            if (!manifest) {
                val probe = com.nanzmusify.nanas.yt.innertube.StreamUrlValidator.validate(
                    url = url,
    ...
    private val fallback: com.nanzmusify.nanas.yt.innertube.InnertubeFallback by lazy {
        com.nanzmusify.nanas.yt.innertube.InnertubeFallback()
    }
     ...
     *    ganti klien di [PlayerClientLadder] (NanzMusify tetap anonim, tanpa cookie).
    ...
    private fun noteExtractorFailure(videoId: String, e: Exception) {
        ...
        if (message.contains("SABR", ignoreCase = true)) {
            com.nanzmusify.nanas.yt.innertube.PlayerClientLadder.noteExtractorSabr(
    ...
    private fun fallbackResolve(
        ...
        } catch (e: com.nanzmusify.nanas.yt.innertube.StreamUnavailableException) {
     ...
     * klien anonim (HLS tidak menuntut poToken GVS). NanzMusify memutarnya lewat
    ...
    suspend fun quickPicks(seedVideoId: String?): List<NanzMusifyTrack> = withContext(Dispatchers.IO) {
    ...
    suspend fun movement(query: String, limit: Int = 14): List<NanzMusifyTrack> = withContext(Dispatchers.IO) {
        val primary = runCatching {
    ...
    private fun mapInfoItem(item: StreamInfoItem): NanzMusifyTrack? {
        val url = item.url ?: return null
        val duration = item.duration
        val vid = videoIdOf(url)
        return NanzMusifyTrack(
            videoId = vid,
    ...
    suspend fun trending(country: String): List<NanzMusifyTrack> = withContext(Dispatchers.IO) {
        runCatching {
    ...
    data class StreamProbe(
        ...
        val ladder: com.nanzmusify.nanas.yt.innertube.LadderProbe,
    ...
    suspend fun probeStream(videoId: String): StreamProbe = withContext(Dispatchers.IO) {
        ...
        val ladderProbe = runCatching { fallback.probeBlocking(id, defaultQuality) }.getOrElse { e ->
            com.nanzmusify.nanas.yt.innertube.LadderProbe(
                videoId = id,
                usableClient = null,
                audioUrlFound = false,
                manifestClient = null,
                ...
                totalMs = 0L,
                attempts = listOf(
                    com.nanzmusify.nanas.yt.innertube.ProbeAttempt(
                        client = "-",
        ...
        StreamProbe(
            ...
            ladder = ladderProbe,
            diagnostics = com.nanzmusify.nanas.yt.innertube.PlayerClientLadder.report(),
        )
    }
    ...
    private fun fetchInnertubeKey(): String {
        ...
        val key = runCatching {
            val req = Request.Builder()
                ...
                .header("User-Agent", NanzMusifyHttp.USER_AGENT)
                .get().build()
            NanzMusifyHttp.streamClient.newCall(req).execute().use { resp ->
    ...
    private fun fetchChartsTopArtists(gl: String): List<ArtistCard> {
        ...
        val root = NanzMusifyHttp.streamClient.newCall(req).execute().use { resp ->
