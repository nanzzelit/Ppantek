 ...
 */
package com.nanzmusify.nanas.data.settings
...
private val Context.nanzmusifyDataStore by preferencesDataStore(name = "nanzmusify_settings")
...
data class NanzMusifySettings(
    ...
    // otomatis diabaikan NanzMusifyTheme, jadi SISTEM jatuh ke skema gelap/terang.
    val themeMode: ThemeMode = ThemeMode.SYSTEM,
    val reduceMotion: Boolean = false,
    val audioQuality: AudioQuality = AudioQuality.BALANCED,
    val autoplayRelated: Boolean = true,
    /** Nama yang disapa di Home (kosong = default "Nanas"). */
...
class SettingsRepository(private val context: Context) {
    ...
    val settings: Flow<NanzMusifySettings> = context.nanzmusifyDataStore.data.map { prefs ->
        NanzMusifySettings(
    ...
    suspend fun setThemeMode(mode: ThemeMode) {
        context.nanzmusifyDataStore.edit { it[Keys.THEME] = mode.name }
    }

    suspend fun setReduceMotion(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.REDUCE_MOTION] = value }
    }

    suspend fun setAudioQuality(value: AudioQuality) {
        context.nanzmusifyDataStore.edit { it[Keys.AUDIO_QUALITY] = value.name }
    }

    suspend fun setAutoplayRelated(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.AUTOPLAY_RELATED] = value }
    }

    suspend fun setDisplayName(value: String) {
        context.nanzmusifyDataStore.edit { it[Keys.DISPLAY_NAME] = value.trim().take(24) }
    }

    suspend fun setAccentArgb(value: Int) {
        context.nanzmusifyDataStore.edit { it[Keys.ACCENT_ARGB] = value }
    }

    suspend fun setFontKey(value: String) {
        context.nanzmusifyDataStore.edit { it[Keys.FONT_KEY] = value }
    }

    suspend fun setDynamicColor(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.DYNAMIC_COLOR] = value }
    }

    suspend fun setArtworkAccent(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.ARTWORK_ACCENT] = value }
    }

    suspend fun setMotionBlur(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.MOTION_BLUR] = value }
    }

    suspend fun setHistoryAutoQueue(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.HISTORY_AUTO_QUEUE] = value }
    }

    suspend fun setLyricsOffsetMs(value: Int) {
        // Batas wajar: ±10 detik — lebih dari itu pasti salah tekan.
        context.nanzmusifyDataStore.edit { it[Keys.LYRICS_OFFSET_MS] = value.coerceIn(-10_000, 10_000) }
    }

    suspend fun setKugouEnabled(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.KUGOU_ENABLED] = value }
    }

    suspend fun setSkipSilence(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.SKIP_SILENCE] = value }
    }

    suspend fun setSkipSilenceInstant(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.SKIP_SILENCE_INSTANT] = value }
    }
    ...
    suspend fun setPlaybackSpeed(value: Float) {
        ...
        context.nanzmusifyDataStore.edit { it[Keys.PLAYBACK_SPEED] = snapped }
    }

    suspend fun setPitchSemitones(value: Int) {
        context.nanzmusifyDataStore.edit { it[Keys.PITCH_SEMITONES] = value.coerceIn(-12, 12) }
    }

    suspend fun setNormalizeAudio(value: Boolean) {
        context.nanzmusifyDataStore.edit { it[Keys.NORMALIZE_AUDIO] = value }
    }
}
