...
debugging, build CI) dan membuat NanzMusify bekerja hari ini. Mengubah salah satunya
...
- Semua trafik on-device langsung ke Google (`youtubei/v1` + `googlevideo.com`) dan
  penyedia lirik; tidak ada backend NanzMusify.
...
- **Unduhan menolak URL `validated = false`** (`download/NanzMusifyDownloadManager.kt`).
...
- **Room 2.8.4 + KSP** untuk DB lokal; **DataStore Preferences** untuk setelan
  (`data/settings/SettingsRepository.kt` → `NanzMusifySettings`). Jangan menambah
...
- `lyrics/LyricsRepository.kt` mencoba `NanzMusifyLyricsProvider` berurutan menurut
...
- Rantai processor audio wajib mempertahankan urutan: detektor hening NanzMusify →
...
- `LICENSE` = teks GPL-3.0 lengkap; **setiap** berkas Kotlin (56 berkas) dimulai dengan
  header `Copyright (C) 2026 Nanas` + `SPDX-License-Identifier: GPL-3.0-only`.
