 ...
 */
package com.nanzmusify.nanas.yt.innertube

import android.util.Log
import com.nanzmusify.nanas.data.model.NanzMusifyTrack
import com.nanzmusify.nanas.data.settings.AudioQuality
import com.nanzmusify.nanas.yt.NanzMusifyHttp
import com.nanzmusify.nanas.yt.ResolvedAudio
import com.nanzmusify.nanas.yt.innertube.InnertubeRequest.Client
 ...
 * diblokir/dihapus" — dua hal yang membutuhkan tindakan berbeda. NanzMusify anonim
...
data class LadderProbe(
    ...
    /** Klien yang memberi `hlsManifestUrl` (bisa diputar — NanzMusify mendukung HLS). */
...
class InnertubeFallback {
    ...
    private val http = NanzMusifyHttp.extractClient
    private val ua = NanzMusifyHttp.USER_AGENT
     ...
     *  2. **Manifest HLS** (perluasan NanzMusify, bukan dari Meld) — bila tidak ada satu pun
     ...
     *     langsung ditutup; NanzMusify memutarnya lewat `media3-exoplayer-hls`.
     *
    ...
    suspend fun related(videoId: String, limit: Int = 25): List<NanzMusifyTrack> =
    ...
    private fun parseRelated(root: JSONObject, limit: Int): List<NanzMusifyTrack> {
        val out = ArrayList<NanzMusifyTrack>(limit)
        ...
        for (i in 0 until results.length()) {
            ...
            out += NanzMusifyTrack(vid, title, artist, durationSec = duration, thumbnailUrl = thumb)
    ...
    suspend fun search(query: String, filter: com.nanzmusify.nanas.data.model.SearchFilter, limit: Int = 30): List<NanzMusifyTrack> =
        withContext(Dispatchers.IO) {
            ensureReady()
            runCatching {
     ...
     */
    private fun searchParams(filter: com.nanzmusify.nanas.data.model.SearchFilter): String? = when (filter) {
        com.nanzmusify.nanas.data.model.SearchFilter.SONGS -> "EgWKAQIIAWoKEAkQBRAKEAMQBA%3D%3D"
        com.nanzmusify.nanas.data.model.SearchFilter.VIDEOS -> "EgWKAQIQAWoKEAkQChAFEAMQBA%3D%3D"
        com.nanzmusify.nanas.data.model.SearchFilter.ALBUMS -> "EgWKAQIYAWoKEAkQChAFEAMQBA%3D%3D"
        com.nanzmusify.nanas.data.model.SearchFilter.ARTISTS -> "EgWKAQIgAWoKEAkQChAFEAMQBA%3D%3D"
        com.nanzmusify.nanas.data.model.SearchFilter.PLAYLISTS -> "EgeKAQQoAEABagoQAxAEEAoQCRAF"
        com.nanzmusify.nanas.data.model.SearchFilter.PODCASTS -> "EgWKAQJQAWoKEAkQChAFEAMQBA%3D%3D"
        com.nanzmusify.nanas.data.model.SearchFilter.ALL -> null
    }

    private fun parseSearch(root: JSONObject, limit: Int): List<NanzMusifyTrack> {
        val out = ArrayList<NanzMusifyTrack>(limit)
    ...
    suspend fun playlist(playlistId: String, limit: Int = 200): List<NanzMusifyTrack> =
        withContext(Dispatchers.IO) {
            ensureReady()
            runCatching {
    ...
    private fun parsePlaylist(root: JSONObject, limit: Int): List<NanzMusifyTrack> {
        val out = ArrayList<NanzMusifyTrack>(limit)
        val contents = root.optJSONObject("contents")
        ...
        for (t in 0 until contents.length()) {
            ...
            for (s in 0 until sections.length()) {
                ...
                for (i in 0 until items.length()) {
                    ...
                    out += NanzMusifyTrack(videoId, title, artist, durationSec = duration, thumbnailUrl = thumb)
                    if (out.size >= limit) return out
                }
            }
        }
