...
/**
 * DataSource yang menyelesaikan skema `nanzmusify://audio/{videoId}` menjadi:
...
class ResolvingDataSource(
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        val resolvedSpec: DataSpec = if (uri.scheme == NANZMUSIFY_SCHEME) {
            val videoId = uri.lastPathSegment
                ?: throw IOException("URI nanzmusify tidak valid: $uri")
            ...
            val target = when {
                !local.isNullOrBlank() && java.io.File(local).exists() ->
                    ...
                    resolved.loudnessDb?.let { db ->
                        com.nanzmusify.nanas.player.audio.LoudnessStore.record(videoId, db)
                    }
    ...
    class Factory(
        ...
        override fun createDataSource(): DataSource {
            val okHttpFactory = OkHttpDataSource.Factory(NanzMusifyHttp.streamClient)
    ...
    companion object {
        const val NANZMUSIFY_SCHEME = "nanzmusify"
        fun uriOf(videoId: String): Uri = Uri.parse("$NANZMUSIFY_SCHEME://audio/$videoId")
    }
}
 ...
 * NanzMusify memakai resolusi malas (`nanzmusify://audio/{id}`) agar antrean terbentuk
