...
// NANZMUSIFY adalah aplikasi dark-first: hitam pekat + crimson logo.
...
// Android 12+ tanpa merusak token lama — lihat [nanzmusifyPaletteFromScheme].
...
private fun darkScheme(p: NanzMusifyPalette) = darkColorScheme(
    primary = p.crimson,
...
private fun lightScheme(p: NanzMusifyPalette) = lightColorScheme(
    primary = p.crimson,
    onPrimary = p.textBright,
    primaryContainer = p.crimson,
    onPrimaryContainer = p.textBright,
...
fun NanzMusifyTheme(
    ...
    val accent = if (accentArgb != -1) Color(accentArgb) else NanzMusifyCrimsonDefault
    ...
    val scheme = remember(context, dark, black, accent, accentOverride, materialYou) {
        val base = when {
            ...
            dark -> darkScheme(if (black) nanzmusifyBlackPalette(accent) else nanzmusifyDarkPalette(accent))
            else -> lightScheme(nanzmusifyLightPalette(accent))
        }
    ...
    val palette = remember(scheme, dark, black, accent, accentOverride, materialYou) {
        ...
        when {
            ...
            materialYou -> nanzmusifyPaletteFromScheme(scheme, effectiveAccent, blackBase = black)
            black -> nanzmusifyBlackPalette(effectiveAccent)
            dark -> nanzmusifyDarkPalette(effectiveAccent)
            else -> nanzmusifyLightPalette(effectiveAccent)
        }
    }

    val typography = remember(fontKey) { nanzmusifyTypography(fontFamilyForKey(fontKey)) }
    ...
    CompositionLocalProvider(
        LocalNanzMusifyPalette provides palette,
        LocalReduceMotion provides reduced,
    ) {
        MaterialTheme(
            colorScheme = scheme,
            typography = typography,
            shapes = NanzMusifyShapes,
            content = content,
        )
    }
}
